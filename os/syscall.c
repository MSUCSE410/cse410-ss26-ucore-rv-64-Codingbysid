...
// LAB5: (3) Deadlock Detect Algorithm
...
// LAB5: (2) Define function enable_deadlock_detect here
uint64 sys_enable_deadlock_detect() {
...
int sys_mutex_create(int blocking)
{
    ...
    // LAB5: (4-1) Initialize available pool
...
int sys_mutex_lock(int mutex_id)
{
    ...
    p->request[tid][mutex_id]--;
    p->allocation[tid][mutex_id]++;
    p->available[mutex_id]--;
...
int sys_mutex_unlock(int mutex_id)
{
    ...
    // LAB5: (4-1) Restore resources
    struct proc *p = curr_proc();
    int tid = curr_thread()->tid;
    
    if (p->allocation[tid][mutex_id] > 0) {
        p->allocation[tid][mutex_id]--;
    }
    p->available[mutex_id]++;
...
int sys_semaphore_up(int semaphore_id)
{
    ...
    if (p->allocation[tid][semaphore_id] > 0) {
        p->allocation[tid][semaphore_id]--;
    }
    p->available[semaphore_id]++;
...
void syscall()
{
    ...
    case SYS_condvar_wait:
        ...
        break;
    // LAB5: (2) Route the syscall
    case SYS_enable_deadlock_detect:
        ret = sys_enable_deadlock_detect();
        break;
    default:
        ret = -1;
